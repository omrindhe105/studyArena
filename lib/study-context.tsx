...
export function StudyProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<StudyState>({
    ...
    revisionNotes: "",
    studyHistory: [
      { date: "2026-03-10", hoursStudied: 4.5, keyPoints: ["Data structures", "Algorithms"] },
      { date: "2026-03-09", hoursStudied: 5.2, keyPoints: ["Operating systems", "Memory management"] },
      { date: "2026-03-08", hoursStudied: 3.8, keyPoints: ["Databases", "SQL"] },
      { date: "2026-03-07", hoursStudied: 6.1, keyPoints: ["Networking", "TCP/IP"] },
      { date: "2026-03-06", hoursStudied: 4.0, keyPoints: ["System design"] },
      { date: "2026-03-05", hoursStudied: 5.5, keyPoints: ["Distributed systems"] },
      { date: "2026-03-04", hoursStudied: 4.8, keyPoints: ["Algorithms review"] },
    ],
    streak: 7,
      ...
      interval = setInterval(() => {
        setState((prev) => {
          if (prev.timerSeconds > 0) {
            ...
            return { ...prev, timerMinutes: prev.timerMinutes - 1, timerSeconds: 59 }
          } else {
            // Timer completed
  ...
  const resetTimer = useCallback(() => {
    setState((prev) => ({
      ...prev,
      isRunning: false,
      timerMinutes: prev.isPomodoroMode ? (prev.isBreak ? 5 : 25) : prev.customMinutes,
      timerSeconds: 0,
    }))
  }, [])
  ...
  const addStudyTime = useCallback((hours: number) => {
    setState((prev) => ({ ...prev, todayHoursStudied: prev.todayHoursStudied + hours }))
  }, [])
